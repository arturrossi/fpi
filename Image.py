...
class Image():
    ...
    def getScaled(self, shape=None, shape_xy=None):
        ...
        assert((shapeis or shapexyis) and not (shapeis and shapexyis)
    ...
    def _splatAtPixCoord(self, im, location=[0, 0], **kwargs):
        ...
        if(im.n_channels < 4):
            self.data[region0[0]:region0[1],
                      region1[0]:region1[1], :] = im.data
            return
        if(im.n_channels == 4):
...
###########################adapted from https://gist.github.com/turicas/1455973##########################
    ...
    def _vbmarker(self):
        ...
        if (scaleval < 1.0):
            vbms = vbm.getScaled(
                shape=[int(vbm.shape[0] * scaleval), int(vbm.shape[1] * scaleval), vbm.shape[2]])
        else:
            vbms = vbm.clone()
        margin = min(vbm.width, vbm.height, int(
            VBMMARGIN * self.width), int(VBMMARGIN * self.height))
